...
from dataset import NYUv2, cityscapes, ACDC, VOC2012
...
def train(args, model):
    ...
    if args.dataset == 'cityscapes':
        ...
        dataset_train = VOC2012(args.datadir, co_transform, 'train')
        dataset_val = VOC2012(args.datadir, co_transform_val, 'val')
    ...
    if args.model.startswith('Segformer'):
        optimizer = AdamW(model.parameters(), 5e-5, (0.9, 0.999), eps=1e-08, weight_decay=0.01)
        scheduler = PolynomialLRDecay(optimizer, max_decay_steps=1500, end_learning_rate=1e-5, power=1.0)
    else:
        assert 'model not supported'
    start_epoch = 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default="VOC", choices=['VOC', 'cityscapes'])
