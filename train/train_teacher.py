...
from models.Segformer import mit_b0, mit_b1, mit_b4, mit_b2, mit_b3
...
def train(args, model):
    ...
    if args.dataset == 'cityscapes':
        ...
        savedir = f'../datalogs/VOC2012/save_mit_b2_teacher2/Testbatch{args.batch_size}-VOC/Baseline'
    else:
        assert 'Dataset does not exist'
    ...
    if args.model.startswith('Segformer'):
        optimizer = AdamW(model.parameters(), 4e-5, (0.9, 0.999), eps=1e-08, weight_decay=0.01)
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        if args.visualize:
            board.add_doubleline(epoch=epoch, val_loss=average_epoch_loss_val, train_loss=average_epoch_loss_train,
                                 title='Losses', win=f'Losses {args.model} {pretrained}')
...
def main(args):
    ...
    if args.model.startswith('Segformer'):
        if args.model.endswith('B0'):
            model = mit_b0(num_classes=NUM_CLASSES, image_size=(args.height, args.width))
            path = 'ckpt_pretrained/mit_b0.pth'
        elif args.model.endswith('B3'):
            model = mit_b2(num_classes=NUM_CLASSES, image_size=(args.height, args.width))
            path = r'.\segformer_b2_backbone_weights.pth'
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="SegformerB3")
